...
int main()
{
    ...
    while (!glfwWindowShouldClose(window) && !inputManager.isPressed(InputId::ESCAPE))
    {
        ...
        if(inputManager.isPressed(InputId::L_CLICK))
            std::cout << "Left click" << std::endl;
        if(inputManager.isPressed(InputId::R_CLICK))
...
void initInput(InputManager<InputId>& inputManager)
{
    ...
    inputManager.addKeyInput(InputId::FORWARD,GLFW_KEY_W,ImEnumDetectionType::NO_REPEAT);
    ...
    inputManager.addMouseInput(InputId::L_CLICK,GLFW_MOUSE_BUTTON_1,ImEnumDetectionType::PRESSED);
    inputManager.addMouseInput(InputId::R_CLICK,GLFW_MOUSE_BUTTON_2,ImEnumDetectionType::PRESSED);
